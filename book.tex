...
This is \hyperref[mnstr:foo]{foo's} home, a hovel of mud and sticks.
...
\begin{DndSidebar}[float=b,width=\textwidth + 8pt]{Behold the DndSidebar!}
...
\section{Tables}
The |DndTable| colours the even rows and is set to the width of a line by default.
...
\subsection{Foo}
Foo is an evil devil that sometimes think he is up to no good. But whenever he goes to a party he comes back drunk and with a few friends richer. If that doesn't sound like someone you want to be friends with, I don't know.

% Monster stat block
\label{mnstr:foo}
\begin{DndMonster}[float=ht]{Monster Foo}
...
\paragraph{Creating a stat block}
The |DndMonster| environment is used to typeset monster and NPC stat blocks. The module supplies many functions to easily typeset the contents of the stat block. nani
